...
func loadAllRules() ([]Rule, error) {
	...
	//need to change the tags of all the "Slack" to the appropriate tags(I didn't know what is the appropriate tag for each one)
	allRules = append(allRules, Rule{Rule: *rules.SlackBotToken(), Tags: []string{TagApiToken}})
	...
	allRules = append(allRules, Rule{Rule: *rules.SlackConfigurationRefreshToken(), Tags: []string{TagAccessToken}})
