...
- ruleId: 01ab7659-d25a-4a1c-9f98-dee9d0cf2e70 # REQUIRED: unique id, must match default rule id to override that default rule. Rule ids can be used as values in --rule and --ignore-rule flags
  ...
  allowLists: # allowed values to ignore if matched
    - description: Allowlist for Custom Rule
      ...
      regexTarget: match - # Can be 'match', 'line' or simply empty/omitted. If 'match', regexes in allowList are tested against the rule.Regex match. If 'line', against the line where the secret was found. If empty/omitted, against the secret itself. Keep in mind, if the rule.Regex has capture groups, the secret is found by one of those capture groups (first capture group by default, or the capture group identified by, if defined, rule.secretGroup) 
