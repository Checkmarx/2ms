...
func FilterRules(selectedList, ignoreList, specialList []string,
	...
	addCustomRules(selectedRules, customRules)
...
func addCustomRules(defaultRules []*ruledefine.Rule, customRules []*ruledefine.Rule) {
	for _, customRule := range customRules {
		ruleMatch := false
		for i := range defaultRules {
			if defaultRules[i].RuleID == customRule.RuleID {
				customRule.BaseRuleID = defaultRules[i].BaseRuleID
				defaultRules[i] = customRule
				ruleMatch = true
				break
			}
		}
		if !ruleMatch {
			defaultRules = append(defaultRules, customRule)
		}
	}
}
