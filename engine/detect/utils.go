...
func filter(findings []report.Finding, redact uint) []report.Finding {
	...
	for i := range findings {
		...
		include := true
		if strings.Contains(strings.ToLower(f.RuleID), constants.GenericCredentialRuleID) { // generic rule ID
			for j := range findings {
				...
				if f.StartLine == fPrime.StartLine &&
					...
					!strings.Contains(strings.ToLower(fPrime.RuleID), constants.GenericCredentialRuleID) {
