...
func GitlabPatRoutable() *config.Rule {
	return &config.Rule{
		...
		Description: "Identified a GitLab Personal Access Token (routable), risking unauthorized access to GitLab " +
			"repositories and codebase exposure.",
...
func GitlabRunnerAuthenticationTokenRoutable() *config.Rule {
	return &config.Rule{
		...
		Description: "Discovered a GitLab Runner Authentication Token (Routable), posing a risk to CI/CD pipeline " +
			"integrity and unauthorized access.",
